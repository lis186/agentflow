...
2. Launch the runner with literal arguments. Record profile, model, effort, active mode, process, transport, output, clone-change, and result-file facts.
   When `ag.json` sets `metrics: auto` or `metrics: ccxray`, pass the Ask id as `task`, the stage as `role`, the project name, and the `ag.json` path; the runner routes Claude, Codex, and Grok through a running ccxray and reports `telemetry` with legal non-null reasons `no_task`, `ccxray_not_found`, `ccxray_too_old`, `unsupported_executable`, `custom_base_url`, `config_unreadable`, or `telemetry_error`, which never blocks a worker, and relays `guidance` once to the owner when required ccxray is missing. After collecting a result whose `telemetry.status` is `active`, run `node metrics.js ccxray-summary --task <Ask id> --role <stage> --project <name> --config <ag.json> --format devlog` and put its output verbatim in that result's RUN body; before closeout, run it once more without `--role` and put that output verbatim under the final report in the Reply. Never retype, round, translate, or estimate the figures; keep the `~`/`+` cost marks and the `as of` stamp (a `+` total is a lower bound, `~` leans on default rates); record an `unavailable` line as-is. The closeout total includes the coordinator when the host itself runs through ccxray (`ccxray claude`, `ccxray codex`, or `ANTHROPIC_BASE_URL` pointing at it); a `host_not_proxied` line means the host bypassed ccxray.
