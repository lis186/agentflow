...
2. Launch the runner with literal arguments. Record profile, model, effort, active mode, process, transport, output, clone-change, and result-file facts.
   When `ag.json` sets `metrics: auto` or `metrics: ccxray`, pass the Ask id as `task`, the stage as `role`, the project name, and the `ag.json` path; the runner routes Claude, Codex, and Grok through a running ccxray and reports `telemetry` with legal non-null reasons `no_task`, `ccxray_not_found`, `ccxray_too_old`, `unsupported_executable`, `custom_base_url`, `config_unreadable`, or `telemetry_error`, which never blocks a worker, and relays `guidance` once to the owner when required ccxray is missing. After each worker attempt exits, run `ccxray-summary --attempt --task <Ask> --role <stage> --config <ag.json> --format devlog` and paste its output verbatim into the RUN body (one line, plus a `gaps:` line when a mark is present); at closeout run `ccxray-summary --cumulative --task <Ask> --config <ag.json> --format devlog` and paste the block verbatim into the Reply; never retype. Receipts live under `.agentflow/evidence/ccxray/` and are the audit record; all figures are modeled from recorded rates, not invoices.
